 ...
 */
Ext.define('dht.ux.plugin.ClearButton', {
  ...
  repositionClearButton: function() {
    ...
    const clearButtonPosition = this.calculateClearButtonPosition(
      this.textField
    );
  ...
  calculateClearButtonPosition: function(textField) {
    const positions = textField.inputEl.getBox(true, true);
    const top = positions.y;
    let right = positions.x;
  ...
  fieldHasScrollBar: function() {
    ...
    const { inputEl } = this.textField;
    const overflowY = inputEl.getStyle('overflow-y');
  ...
  isButtonCurrentlyVisible: function() {
    ...
    const cachedVisible = Ext.core.Element.data(
  ...
  updateClearButtonVisibility: function() {
    const oldVisible = this.isButtonCurrentlyVisible();
    const newVisible = this.shouldButtonBeVisible();
    ...
    if (oldVisible !== newVisible) {
      ...
      if (!(this.isTextArea && Ext.isGecko) && !Ext.isIE) {
        ...
        const deltaPaddingRight =
          clearButtonEl.getWidth() - this.clearButtonEl.getMargin('l');
        const currentPaddingRight = this.textField.inputEl.getPadding('r');
        const factor = newVisible ? +1 : -1;
        this.textField.inputEl.dom.style.paddingRight = `${currentPaddingRight +
          factor * deltaPaddingRight}px`;
      }
    }
  },
