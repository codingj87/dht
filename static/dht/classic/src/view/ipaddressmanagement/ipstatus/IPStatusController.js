...
Ext.define('dhcp.view.ipaddressmanagement.ipstatus.IPStatusController', {
  extend: 'Ext.app.ViewController',
  alias: 'controller.ipstatus',

  requires: ['dhcp.view.windows.assignediplistwindow.AssignedIPListWindow'],
  ...
  onSearch: function() {
    const form = this.lookupReference('searchForm'),
      subnet = form.getValues()['subnet'];

    if (form.isValid()) {
      if (subnet && dhcp.checkSearchIpForm(subnet, 'normal')) return;
    ...
    dht.ajax(
      ...
      function(response) {
        if (response.success === true) {
          new dhcp.view.windows.assignediplistwindow.AssignedIPListWindow({
            animateTarget: cp,
            viewModel: {
              data: {
